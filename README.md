...
| # | Issue | Status / leads |
|---|---|---|
| 1 | **Saving/popups freeze the game** ("not responding") + the mouse does nothing / lands wrong | ✅ **FIXED & DEPLOYED (2026-08-10) — [docs/SAVE-FREEZE-ROOT-CAUSE.md](docs/SAVE-FREEZE-ROOT-CAUSE.md).** The shell hit-tests raw `GetCursorPos` screen coords against its 640×480 button rects; on a stretched window the cursor is never "inside" a button and the popup poll loop spins forever ("not responding"). Fix: the **`u32x` USER32 proxy** (`i76-uncap-lab/src/u32x.c`, deploy via `tools/instruments/deploy-shellfix.ps1`) translates screen↔640×480 and guarantees an in-range coord so it can't hang. **Verified**: reproduced the freeze on the portable install and defeated it; the real "Overwrite?" popup now answers from a click where it visually is; a full save completes. Also gives correct full-screen mouse everywhere |
...
| 1c | **Save-name text box is hard to type into** | A real, separate shell bug: it drops keystrokes (auto-test lost 4 of 5 chars). Not fixed by the cursor proxy; would need a keyboard-path intercept. Deferred — see the doc |
