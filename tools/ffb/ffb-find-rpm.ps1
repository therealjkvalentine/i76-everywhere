...
$drive = Capture "PHASE 3: DRIVE THROUGH THE GEARS" $DriveSeconds `
    ("Now accelerate from a stop to top speed in a straight line.`n" +
     "This is only used to describe the candidates, not to pick them.")
...
# ---- gear hunt -----------------------------------------------------------
# Opposite signature to RPM: still while parked, stepping while driving, and
# living in a very small integer range throughout.
Write-Host ""
Write-Host "=== gear candidates: int, constant while parked, stepping up under way ===" -ForegroundColor Cyan
$gearHits = New-Object System.Collections.Generic.List[object]
for ($i = $NSLOT; $i -lt $total; $i++) {
    $pv = $parked | ForEach-Object { $_.V[$i] }
    $pmn = ($pv | Measure-Object -Minimum).Minimum
    $pmx = ($pv | Measure-Object -Maximum).Maximum
    if ($pmn -ne $pmx) { continue }              # must be constant while parked
    if ($pmn -lt 0 -or $pmn -gt 6) { continue }  # a gear index, not a pointer
    $dv = $drive | ForEach-Object { $_.V[$i] }
    if ($dv.Count -lt 4) { continue }
    $dmn = ($dv | Measure-Object -Minimum).Minimum
    $dmx = ($dv | Measure-Object -Maximum).Maximum
    if ($dmx -le $dmn) { continue }              # must actually change under way
    if ($dmx -gt 8) { continue }
    $steps = ($dv | Sort-Object -Unique).Count
    if ($steps -gt 8) { continue }               # a gearbox has a few discrete states
    $gearHits.Add([pscustomobject]@{
        Name = (SlotName $i); Parked = $pmn; DMin = $dmn; DMax = $dmx; Steps = $steps })
}
if ($gearHits.Count -eq 0) {
    Write-Host "  none - no small integer stepped between parked and driving." -ForegroundColor DarkGray
} else {
    foreach ($g in ($gearHits | Sort-Object Steps -Descending | Select-Object -First 12)) {
        Write-Host ("  {0,-12}  parked {1,3}   driving {2,3}..{3,-3}  {4} distinct values" -f `
