...
function Render-Scenario {
    ...
    for ($i = 0; $i -lt $n; $i++) {
        ...
        $s = [pscustomobject]@{
            T=$t; Speed=$sp; SpeedMph=$sp*2.23694; Steer=0.0; Throttle=[double]$st.Throttle
            YawRate=0.0; AngVelX=0.0; AngVelZ=0.0; Vy=$vy
            LongG=$lg; LongAccel=$la; LatG=0.0; LatAccel=0.0
            HeaveAccel=($vy * 20.0); TravelPitch=0.0
            ExpectedYaw=0.0; Understeer=0.0; Oversteer=0.0
            Jolt=$jolt; Firing=$false; Braking=($lg -lt -0.05); Airborne=$false
    ...
    $sets = if ($Solo) {
        @(@{S='all';K=@()}, @{S='engine';K=@('r','i','w','h')}, @{S='road';K=@('e','i','w','h')},
          @{S='heave';K=@('e','r','i','w')})
    } else { @(@{S='all';K=@()}) }

    foreach ($set in $sets) {
        $e2=$ea.Clone(); $r2=$ra.Clone(); $i2=$ia.Clone(); $w2=$wa.Clone(); $h2=$ha.Clone()
        foreach ($k in $set.K) {
            switch ($k) {
        ...
        $pcm = [LfeCore]::RenderLive($ef,$e2,$rf,$r2,$i2,$w2,$h2,
